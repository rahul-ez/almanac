...
2. **Student Council & Admins**: A centralized **Control Center** to schedule events, reserve rooms with automated overlap conflict detection, analyze club engagement, and inspect live student registration rosters.

---

## 🖼️ Visual Tour & Key Interfaces

### 1. 🏠 Student Homepage & Campus Pulse
...
### 3. 🎟️ Events Directory & 1-Click Registration
...
### 5. 🏛️ Council Control Center & Live Attendee Rosters
...
* **🧠 Governed NL-to-SQL (Databricks Genie)**: Full multi-table natural language reasoning across 8 Unity Catalog tables with zero hallucination guarantees, action deep-links (`#register:<id>`), and strict out-of-scope boundaries.
* **🛡️ Conflict-Free Room Reservations**: Automated temporal overlap detection (`[start_ts, end_ts)`) ensuring no two clubs can double-book classrooms, auditoriums, or labs.
* **🚀 Blazing Fast Lakehouse Performance**:
...
## 🗄️ Governed Data Model (Unity Catalog)

All data is stored in Delta Lake under the Unity Catalog namespace `campus_companion.campus`:

| Table | Description | Key Columns |
| :--- | :--- | :--- |
| `clubs` | Campus student organizations & societies | `club_id`, `name`, `category`, `active` |
...
| `internships` | Curated campus career & internship postings | `internship_id`, `company_name`, `role_title`, `stipend`, `status` |
...
Run the full backend test suite (106 unit & integration tests):
...
<div align="center">
<b>Almanac</b> — Built with ❤️ for the Campus Community.
</div>
